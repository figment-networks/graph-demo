...
func (d *Driver) StoreTransactions(ctx context.Context, txs []structs.Transaction) error {
	...
	for _, t := range txs {
		...
		if err := d.storeTx(ctx, d.db, t, fee, buff); err != nil {
			return err
		}
...
func (d *Driver) storeTx(ctx context.Context, db *sql.DB, t structs.Transaction, fee []byte, events fmt.Stringer) error {
	tx, err := db.Begin()
	if err != nil {
		return err
	}
